...
export class SVGPathService {
    ...
    getSingleLinkDrawnPath(link: Link, radius: number): string {
        //get all coordinates within the link.
        let allCoords: Coord[] = [];
        for (let joint of link.joints.values()) {
            let modeltoSVGConversion: Coord = this.panZoomService.modelPosToSVGPos(joint._coords);
            allCoords.push(modeltoSVGConversion);
        }
