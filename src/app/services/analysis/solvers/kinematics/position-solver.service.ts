...
export class PositionSolverService {
  ...
  updateAnimationPositions() {
    ...
    for (let solveOrder of this.solveOrders) {
      this.animationPositions.push({correspondingJoints: solveOrder.order, positions: this.getPositions(solveOrder.order, solveOrder.prerequisites as Map<number, IPositionSolverPrerequisite>)})
    }
  }
  ...
  private determineSolveOrder(subMechanism: Map<Joint, RigidBody[]>): ISolveOrder {
    let solveMap: Map<number, IPositionSolverPrerequisite> = new Map();
    ...
    for (let joint of subMechanism.keys()) {
      if (joint.isGrounded && !joint.isInput && joint.type != JointType.Prismatic) {
        ...
        solveMap.set(joint.id, { jointToSolve: joint, solveType: SolveType.Ground } as IPositionSolverPrerequisite);

        solveOrder.push(joint.id);
      } else if (joint.isGrounded && joint.isInput) {
        //input is always first.
        if (joint.type == JointType.Prismatic) {
          solveMap.set(joint.id, { jointToSolve: joint, solveType: SolveType.PrismaticInput } as IPositionSolverPrerequisite);
        } else {
          solveMap.set(joint.id, { jointToSolve: joint, solveType: SolveType.Ground } as IPositionSolverPrerequisite);
        }
    ...
    return { order: solveOrder, prerequisites: solveMap } as ISolveOrder;

  }

  private jointCanBeSolved(joint: Joint, links: RigidBody[], solvedJoints: number[]): IPositionSolverPrerequisite | undefined {
      ...
      for (let neighbor of link.getJoints()) {
        if (solvedJoints.includes(neighbor.id)) {
          ...
          if (neighbor.isInput && neighbor.type != JointType.Prismatic) {
            ...
            } as IPositionSolverPrerequisite
          } else {
  ...
  private getPositions(solveOrder: number[], solveMap: Map<number, IPositionSolverPrerequisite>): Coord[][] {
  ...
  private solveNextJointPosition(prevPositions: Coord[], solveOrder: number[], solvePrerequisite: IPositionSolverPrerequisite, movingForward: number, nextPositions: Coord[]): Coord | undefined {
  ...
  private solveRevInput(prevPositions: Coord[], solveOrder: number[], solvePrerequisite: IPositionSolverPrerequisite, movingForward: number): Coord | undefined {
  ...
  private solvePrisInput(prevPositions: Coord[], solveOrder: number[], solvePrerequisite: IPositionSolverPrerequisite, movingForward: number): Coord | undefined {
  ...
  private solveCircleCircle(prevPositions: Coord[], solveOrder: number[], solvePrerequisite: IPositionSolverPrerequisite, movingForward: number, nextPositions: Coord[]): Coord | undefined {
  ...
  private solveCircleLine(prevPositions: Coord[], solveOrder: number[], solvePrerequisite: IPositionSolverPrerequisite, movingForward: number, nextPositions: Coord[]): Coord | undefined {
